 ...
 * 保存配置到配置文件中 
 */
function saveConfigToFile($configDir, $configFileName, $configData) {
    // 将配置换成JSON格式
    $manageConfigJson = json_encode($configData);
    // 判断管理应用配置目录是否存在
    if (!is_dir($configDir)) {
        // 文件夹不存在，创建文件夹
    ...
    // 修改管理应用目录权限和所有者
    exec("sudo chown www-data:www-data $configDir");
    // 实测必须给www-data rwx权限（也就是7），否则使用file_put_contents写入配置文件无法写入
    exec("sudo chmod 755 $configDir");
    // 管理应用配置文件
    ...
    // 将JSON数据写入文件
    return file_put_contents($configFile, $manageConfigJson);
}
?>